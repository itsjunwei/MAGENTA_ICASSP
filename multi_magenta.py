...
class MAGENTA_ADPIT(nn.Module):
    ...
    def __init__(self, num_classes: int = 13, ntracks: int = 3, class_counts_path: str = "starss_class_counts.npy",
                 use_class: bool = False, use_miangle: bool = False, use_sat: bool = False, ifl: bool = False, neg_mode: str = "none", eps: float = 1e-8):
        super().__init__()
        ...
        assert len(counts) == self.C, f"class_counts mismatch: {len(counts)} vs C={self.C}"
        ...
        if self.ifl:
            print(f"Inverse Frequency Loss!")
        else:
    ...
    def _magenta_tracks_batched(self, pred, targ):
        ...
        # Deviation to Inverse Frequency Loss
        if self.ifl:
            mse = (xp - xt).pow(2) + (yp - yt).pow(2) + (zp - zt).pow(2)
            cw = self.class_weights.view(1,1,1,1, self.C)
            L_active = cw * mse * active

            inact_cw = self.inv_priors.view(1,1,1,1, self.C)
            L_inactive = inact_cw * mse * inactive

            L_tot = L_active + L_inactive
            return L_tot.mean(dim=3)
